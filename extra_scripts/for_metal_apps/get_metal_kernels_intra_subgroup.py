...
def mk_plain_test(finput, prefix, t):
    ...
    rm_and_mkdir(dir_name)
    os.system("cp " + finput + " " + dir_name + "/")
    my_config = Configuration(timeout=20000, workgroups=1, threads_per_workgroup=256, saturation_level=0, subgroup=1)
    #amber_test_generation.generate_amber_test(finput, dir_name+"/"+name)
    #cpp_test_generation.generate_cpp_test(finput, dir_name+"/"+name)
    metal_test_generation.generate_metal_test(finput, dir_name+"/"+name,my_config)
    return get_data(dir_name+"/"+name + ".metal")
...
for i in sizes_alias:
    ...
    #print(c_tests)
    plain_names, plain_kernels = generate_tests(i,c_tests)
    uplain_kernels = []
    uchunked_kernels = []
    urr_kernels = []
