...
int main(int argc, char **argv) {
  ...
  const unsigned int total_threads = 96;
  const unsigned int bufferSize = (1 * sizeof(int));
  ...
  for (int t = t_num; t < t_num+1; t++) {
	...
	hBufferY[i] = 0;
      ...
      //*(kernels[t])<<<1024,1024,1024>>>(dBufferX, dBufferY, dBufferCounter);
      test_launcher_kernel<<<1,total_threads>>>(dBufferX, dBufferY, dBufferCounter, host_function_ptr);
      
      gpuErrchk(cudaDeviceSynchronize());
      
      gpuErrchk(cudaMemcpy(hBufferCounter, dBufferCounter, sizeof(uint), cudaMemcpyDeviceToHost));
      assert(*hBufferCounter == total_threads);
      //printf("found %d\n", *hBufferCounter);
    }
  }
