...
def write_cpp_prologue(output, timeout, threads_per_workgroup, workgroups, num_testing_threads, saturation_level,
    ...
    output.write("void testKernel(atomic_uint * x, atomic_uint * y, atomic_int* count, uint tid) {\n")
    ...
    if saturation_level == 1:
        ...
        output.write("\n")
        output.write("\tint total_num_threads = " + "MAX_THREADS" + ";\n")
...
def write_cpp_thread_program(output, thread_instructions, thread_number, number_of_testing_threads, saturation_level,
    ...
    output.write("\t   if (terminate == 1 || (atomic_load_explicit(cpu_terminate,memory_order_relaxed) == 1)) {\n")
    output.write("\t   break;\n")
    output.write("\t}\n")
...
def handle_atomic_exchange_branch(output, check_value, exchange_value, instruction_address, saturation_level,
    ...
    if saturation_level == 0:
        ...
        if int(memory_location) == 0:
            ...
            output.write("\t\tif (atomic_exchange_explicit(x, (uint)" + exchange_value + ", memory_order_relaxed) == " + check_value + ") { \n")
        else:
            output.write("\t\tif (atomic_exchange_explicit(y, (uint)" + exchange_value + ", memory_order_relaxed) == " + check_value + ") { \n")
            #output.write("\t\tif (atomicExchange(test.y, " + exchange_value + ") == " + check_value + ") { \n")

    elif saturation_level == 1 or saturation_level == 2:
        # determine whether to write to memory location x[] or memory location y[]
        if int(memory_location) == 0:
            output.write("\t\tif (atomic_exchange_explicit(x + index, (uint)" + exchange_value + ", memory_order_relaxed) ==  " + check_value +
                         ") { \n")
        else:
            output.write("\t\tif (atomic_exchange_explicit(y + index, (uint)" + exchange_value + ", memory_order_relaxed) ==  " + check_value +
                         ") { \n")
...
def handle_amber_check_branch(output, check_value, instruction_address, saturation_level, memory_location, program_end):
    ...
    if saturation_level == 0:
        ...
        if int(memory_location) == 0:
            ...
            output.write("\t\tif (atomic_fetch_add_explicit(x, (uint) 0, memory_order_relaxed) == " + check_value + " ) { \n")
        else:
            output.write("\t\tif (atomic_fetch_add_explicit(y, (uint) 0, memory_order_relaxed) == " + check_value + " ) { \n")
            #output.write("\t\tif (atomicAdd(test.y, 0) == " + check_value + " ) { \n")

    elif saturation_level == 1 or saturation_level == 2:
        # determine whether to write to memory location x[] or memory location y[]
        if int(memory_location) == 0:
            output.write("\t\tif (atomic_fetch_add_explicit(x+index, (uint) 0, memory_order_relaxed) == " + check_value + " ) { \n")
            #output.write("\t\tif (atomicAdd(out_buf1.x[index], 0) == " + check_value + " ) { \n")
        else:
            output.write("\t\tif (atomic_fetch_add_explicit(y+index, (uint) 0, memory_order_relaxed) == " + check_value + " ) { \n")
...
def handle_atomic_store(output, write_value, saturation_level, memory_location):
    ...
    if saturation_level == 0:
        ...
        if int(memory_location) == 0:
            ...
            output.write("\t\tatomic_exchange_explicit(x, (uint)" + write_value + ", memory_order_relaxed);\n")
        else:
            output.write("\t\tatomic_exchange_explicit(y, (uint)" + write_value + ", memory_order_relaxed);\n")
            #output.write("\t\tatomicExchange(test.y, " + write_value + ");\n")

    elif saturation_level == 1 or saturation_level == 2:
        # determine whether to write to memory location x[] or memory location y[]
        if int(memory_location) == 0:
            output.write("\t\tatomic_exchange_explicit(x + index, (uint)" + write_value + ", memory_order_relaxed);\n")
        else:
            output.write("\t\tatomic_exchange_explicit(y + index, (uint)" + write_value + ", memory_order_relaxed);\n")
...
def generate_cpp_test(inputted_file, output_file_name, config=default_config):
    ...
    if output_file_name.endswith(".cpp"):
        print("Script will include the .cpp extension, please provide a different output file name", file=sys.stderr)
        exit(1)
    ...
    output_cpp_file = output_cpp_file + ".cpp"
...
def main():
    if len(sys.argv) != 3:
        print("Please provide a .txt file to parse and the desired name for the outputted cpp file", file=sys.stderr)
        exit(1)
    ...
    generate_cpp_test(input_file, output_file, default_config)
